...
export default function Login() {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      setError('Google ile giriş başarısız. Firebase Google sağlayıcısının aktif olduğundan emin olun.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6 relative overflow-hidden">
      ...
      <div className="absolute top-0 left-0 w-full h-full opacity-20 pointer-events-none">
        <div className="absolute top-1/4 left-1/4 w-[500px] h-[500px] bg-indigo-500 rounded-full blur-[150px]" />
        <div className="absolute bottom-1/4 right-1/4 w-[500px] h-[500px] bg-purple-500 rounded-full blur-[150px]" />
      </div>
      ...
      <div className="bg-white/80 backdrop-blur-xl w-full max-w-md p-12 rounded-[40px] shadow-2xl border border-white relative z-10 text-center">
        ...
        <h2 className="text-3xl font-black text-slate-900 mb-2">Hoş Geldiniz</h2>
        <p className="text-slate-400 mb-10 font-medium">İşletmenizin geleceğine giriş yapın.</p>

        <button
          type="button"
          onClick={handleGoogleSignIn}
          disabled={loading}
          className="w-full py-4 px-5 bg-white text-slate-700 rounded-2xl font-black border border-slate-200 shadow-sm hover:border-indigo-200 hover:shadow-xl hover:shadow-indigo-100 transition-all active:scale-95 disabled:opacity-60 flex items-center justify-center gap-3 group"
        >
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          >
            {loading ? 'GİRİŞ YAPILIYOR...' : "PANEL'E ERİŞ"}
          </button>
        </form>
